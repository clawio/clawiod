...
type testObject struct {
	...
	ocsqlDataController *ocsqlDataController
...
func (o *testObject) setupController(t *testing.T, dirs *config.Directives) {
	...
	o.ocsqlDataController = o.dataController.(*ocsqlDataController)
...
func Test_computeChecksum_withBadChecksum(t *testing.T) {
	...
	err := ioutil.WriteFile(p, []byte("1"), 0644)
	require.Nil(t, err)
	_, err = o.ocsqlDataController.computeChecksum(p)
	require.NotNil(t, err)
}
...
func Test_computeChecksum_withoutFile(t *testing.T) {
	...
	dirs.Data.Simple.Checksum = "md5"
	_, err := o.ocsqlDataController.computeChecksum("/this/does/not/exist/myblob")
	require.NotNil(t, err)
}
...
func Test_computeChecksum_md5(t *testing.T) {
	...
	err := ioutil.WriteFile(p, []byte("1"), 0644)
	require.Nil(t, err)
	checksum, err := o.ocsqlDataController.computeChecksum(p)
	require.Nil(t, err)
