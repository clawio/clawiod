...
func (a *WebDAV) HandleRequest(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + STATUS_URL}, "/")) && r.Method == "GET" {
		a.status(ctx, w, r)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + CAPABILITIES_URL}, "/")) && r.Method == "GET" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.capabilities)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "GET" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.get)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "PUT" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.put)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "MKCOL" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.mkcol)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "OPTIONS" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.options)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "PROPFIND" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.propfind)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "LOCK" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.lock)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "UNLOCK" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.unlock)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "DELETE" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.delete)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "MOVE" {
		a.adisp.AuthenticateRequestWithMiddleware(ctx, w, r, true, a.move)
	} else if strings.HasPrefix(path, strings.Join([]string{a.cfg.GetDirectives().APIRoot, a.GetID() + REMOTE_URL}, "/")) && r.Method == "COPY" {
