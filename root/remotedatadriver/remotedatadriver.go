...
func New(logger levels.Levels, cm root.ContextManager, url string) root.DataDriver {
	url = strings.TrimRight(url, "/")
	return &driver{logger: logger, cm: cm, url: url, client: http.DefaultClient}
}
...
func (c *driver) DownloadFile(ctx context.Context, user root.User, path string) (io.ReadCloser, error) {
	...
	req, err := http.NewRequest("GET", c.url+"/download", nil)
	if err != nil {
		return nil, err
	}
