...
// Package webdav defines the OCWebDAV API to manage the resources using
...
// OCWebDAV is the implementation of the API interface to manage resources using OCWebDAV.
...
func New(id string, adisp adisp.Pat, sdisp sdisp.Pat, cfg config.Config, log logger.Logger) api.API {
	fa := OCWebDAV{
		id:     id,
		adisp:  adisp,
		Pat:    sdisp,
...
func (a *OCWebDAV) HandleRequest(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	path := r.URL.Path
	if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + STATUS_URL}, "/")) && r.Method == "GET" {
		a.status(ctx, w, r)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + CAPABILITIES_URL}, "/")) && r.Method == "GET" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.capabilities)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "GET" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.get)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "PUT" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.put)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "MKCOL" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.mkcol)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "OPTIONS" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.options)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "PROPFIND" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.propfind)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "LOCK" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.lock)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "UNLOCK" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.unlock)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "DELETE" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.delete)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "MOVE" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.move)
	} else if strings.HasPrefix(path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/")) && r.Method == "COPY" {
		a.adisp.ValidateRequestHandler(ctx, w, r, true, a.copy)
	} else {
...
func (a *OCWebDAV) capabilities(ctx context.Context, w http.ResponseWriter, r *http.Request) {
...
func (a *OCWebDAV) copy(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)

	resourcePath := strings.TrimPrefix(r.URL.Path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/"))
	...
	destination = strings.TrimPrefix(destinationURL.Path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID()}, "/")+"/")
...
func (a *OCWebDAV) delete(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
...
func (a *OCWebDAV) get(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
...
func (a *OCWebDAV) head(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
...
func (a *OCWebDAV) lock(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
...
func (a *OCWebDAV) mkcol(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
	resourcePath := strings.TrimPrefix(r.URL.Path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/"))
	...
	err = a.CreateContainer(identity, resourcePath, false)
	if err != nil {
		switch err.(type) {
		case *storage.NotExistError:
			log.Debug(err.Error())
...
func (a *OCWebDAV) move(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
...
func (a *OCWebDAV) options(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
	resourcePath := strings.TrimPrefix(r.URL.Path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/"))
...
func (a *OCWebDAV) propfind(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
	resourcePath := strings.TrimPrefix(r.URL.Path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/"))
...
func getPropFindFromMeta(a *OCWebDAV, meta *storage.MetaData) ([]*responseXML, error) {
...
func getResponseFromMeta(a *OCWebDAV, meta *storage.MetaData) (*responseXML, error) {
...
func (a *OCWebDAV) put(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	log := ctx.Value("log").(logger.Logger)
	identity := ctx.Value("identity").(auth.Identity)
	resourcePath := strings.TrimPrefix(r.URL.Path, strings.Join([]string{a.GetDirectives().APIRoot, a.ID() + REMOTE_URL}, "/"))
	...
	if r.Header.Get("X-Expected-Entity-Length") != "" {
			...
			//return
				   ...
				   //              throw new Exception\Forbidden('This server is not compatible with OS/X finder. Consider using a different OCWebDAV client or webserver.');
				   //          }
	...
	if err != nil {
		...
		switch err.(type) {
		case *storage.NotExistError:
			err = a.PutObject(identity, resourcePath, r.Body, r.ContentLength, verifyChecksum, checksumType, checksum)
			if err != nil {
				switch err.(type) {
				case *storage.NotExistError:
					log.Debug(err.Error())
	...
	err = a.PutObject(identity, resourcePath, r.Body, r.ContentLength, verifyChecksum, checksum, checksumType)
	if err != nil {
		if err != nil {
			switch err.(type) {
			case *storage.NotExistError:
...
func (a *OCWebDAV) unlock(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusNoContent)
}
...
// as in the header.
func (a *OCWebDAV) getChecksumInfo(ctx context.Context, r *http.Request) (string, string, error) {
