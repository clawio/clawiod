...
func (s *svc) infoToPropResponse(info *entities.ObjectInfo) (*responseXML, error) {
	...
	propList = append(propList, getResourceType, getContentLegnth, getContentType, getLastModified)
	...
	response.Href = filepath.Join(s.conf.GetDirectives().Server.BaseURL, s.BaseURL(), "home", info.PathSpec)
	if info.Type == entities.ObjectTypeTree {
		response.Href = filepath.Join(s.conf.GetDirectives().Server.BaseURL, s.BaseURL(), "home", info.PathSpec) + "/"
	}
